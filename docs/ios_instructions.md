...
- Note: You need to point to the Python server, not the node server that hosts the web front end. By default, python server runs on port 8000 by default, so just add :8000 to your IP address. (and you have to make sure this port is accessible through your firewall).
